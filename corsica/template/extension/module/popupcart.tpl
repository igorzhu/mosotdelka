	...
	<?php if ($products || $vouchers) { ?>
			...
			<table>
				...
				<?php foreach ($products as $key => $product) { ?>
					<tr class="row_<?php echo $key; ?>_<?php echo $product['id']; ?>">
						...
						<td class="remove"><i class="fa fa-trash-o" title="<?php echo $button_remove; ?>" onclick="$(this).closest('tr').remove(); cart.remove('<?php echo $product['key']; ?>')" ></i></td>
					</tr>
					<tr class="hr"><td colspan="6"></td></tr>
				<?php } ?>
			</table>
		...
		<div class="popupcart_buttons">
			<?php if($button_shopping_show) { ?>
				<button type="button" class="btn btn-bord" onclick="$('#popupcart_extended').popup('hide')" ><?php echo $button_shopping; ?></button>
			<?php } else { ?>
			...
			<?php } ?>
			<button type="button" class="btn btn-green" onclick="location='<?php echo $checkout; ?>'"><?php echo $button_checkout; ?></button>
		</div>
